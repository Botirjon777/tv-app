...
async function main() {
  ...
  await prisma.menuRoom.deleteMany();
  await prisma.menuHotel.deleteMany();
  ...
  const hotelCount = await prisma.menuHotel.count();
  const roomCount = await prisma.menuRoom.count();
